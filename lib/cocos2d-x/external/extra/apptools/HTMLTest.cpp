...
bool HTMLTest::init()
{
	...
	CCLuaLog("entry HTMLTest::init()-------------------0");
	// font1
	...
	CCLuaLog("entry HTMLTest::init()-------------------2");
	font_catalog->add_hackfont("Marker Felt.ttf", latin_charset(), -1);
	CCLuaLog("entry HTMLTest::init()-------------------22");
	// font3
	font_catalog = FontFactory::instance()->create_font(
	...
	CCLog("entry HTMLTest::init()-------------------str_utf8=%x", str_utf8);
	if (NULL==str_utf8) return true;
...
int HTMLTest::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
	return 1;
}
